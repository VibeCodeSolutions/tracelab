...
- **Status:** ✅ Sammel-Gate-grün — qs-20260518-002 freigabe/minor (1 Minor VC-001-KON ARCH-Doku-Drift lead-direktarbeit gefixt). **FF-Merge-Ready für Phase 2d gesamt** (S0+S1+S2+S3+S4+S5+S5-Tail), wartet auf Admin-Approval.
- **Verlauf:**
  ...
  - 2026-05-18T (Annahme + Worker-Spawn) — belanna: Auftrag angenommen, Klasse sprint mit 6 Deliverables (Migration 0005 + Store + Multi-Source-Ingest + Read-Endpoint + Client+UI + Tests). Worker-Spawn ballard mit ADR-014-Schema-Block verbatim aus ARCH.md, Pre-Hardcoding-Verifikation 6. Anwendung als Pflicht-Erststep, Auto-Stop-Trigger im Brief (Transcript-Extraktion-Marker unklar → scope-reduzieren als Phase-3-Bookmark, kein Eskalations-Stop). Cross-Check-Scope 21. Anwendung, 3-Klauseln-Methodik gilt. Nach Worker-Done → **Tuvok release-qs als Sammel-Gate über Phase 2d gesamt** (S0+S1+S2+S3+S4+S5 = #031-#036-#037 zusammen). Bei Sammel-Gate-grün → routing-abschluss-Bericht an Admin mit FF-Merge-Approval-Frage.
  - 2026-05-18T (Worker-Done) — ballard: alle 6 Deliverables abgearbeitet bis auf Transcript-Tail-Source der scope-reduziert wurde. **(1) Migration 0005** (additive `CREATE TABLE agent_event_refs` + 3 Indexes, .down.sql = `DROP TABLE` — verbatim aus ADR-014 Schema-Block). **(2) Store-Layer** in `internal/store/agents.go`: `AgentEventRef`/`AgentEventRefRow` typed structs, `InsertAgentEventRef(ctx, ref)` mit FK+CHECK-Validation und INSERT OR IGNORE, `AgentEventRefsForSpawn(ctx, spawnID)` ordered ts ASC. **(3) Multi-Source-Ingest:** Wire-Path über alle 3 Sources via Handler.persist (additive `event_refs[]`-Loop) — SDK-Hook + MCP-Push + Transcript-Wire akzeptieren EventRefPayload-Array, `validate()` prüft ref_type/event_id/ts. **Transcript-Tail aktive Emission** = **Phase-3-Bookmark** (plan-konform, Brief erlaubt scope-reduzieren): empirische Marker-Form aus Sample-Transcripts fehlt — JSONL hat keine `events.id`-Marker im toolUseResult, eine spätere Heuristik (z.B. tripwire-event-id im app-log gegen spawn-ts-Window matchen) gehört in eine eigene Architektur-Diskussion, nicht in S5-Tail. Wire-Path ist test-pinned via `TestAgentsIngestThreeSourceEventRefsCollapseOnUniqueTuple`. **(4) Read-Endpoint** `GET /agents/event_refs?spawn_id=…` als 6. Route im `AgentsReadMux` (Bearer-Auth, spawn_id-Format-Guard, JSON-Envelope `{spawn_id, event_refs:[]}` analog `/agents/edges`). **(5) Client-Methode** `AgentsEventRefs(ctx, spawnID)` in `internal/client/agents.go` analog `AgentsEdges` (doRequest-Pattern, Nil→Empty-Slice-Substitution). **(6) UI** in `web/templates/tab_agents.gohtml`: neue **Event-Refs**-Spalte adjacent zur Edges-Spalte (ADR-014 Decision: nicht merged), `tl-agent-event-ref-list` Sub-Liste mit `tl-agent-event-ref-{type}`-Pills (observed/context/caused-by) und `#<event-id>` monospace; 3 neue CSS-Regeln in `web/static/dashboard.css` (additiv, kein bestehender Selector geändert). **MCP-Push agent_event-Tool** in `cmd/mcp/agent_event.go` um optional `event_refs[]`-Array erweitert — `agentEventEventRef`-Struct mirror `EventRefPayload`, Tool-Schema bekommt `WithArray("event_refs", ...)` mit unix-nano-Beschreibung. **Pre-Hardcoding-Verifikation 6. erfolgreiche Anwendung:** (a) `events.id INTEGER PRIMARY KEY AUTOINCREMENT` (migration 0001 Z.9) empirisch bestätigt → FK-Target valid; (b) Migration-Chain 0001/0002/0003 vorhanden + 0004 frei → 0005 KORREKT, Gap akzeptabel laut `migrate()`-Sortier-Logik (lexicographic via version-int sort, keine Sequenz-Pflicht); (c) `agent_event`-Tool-Pattern mit `mailbox_edges[]` 1:1 für `event_refs[]` übernommen (kein neues Pattern erfunden); (d) Transcript-JSONL trägt keinen Marker für `events.id` → scope-reduziert als Phase-3-Bookmark; (e) `ref_type`-Werte verbatim aus ADR-014 übernommen, ARCH.md Consequences NICHT erweitert. **Folge-Edits außerhalb erlaubtem Scope:** `internal/store/sqlite_test.go` zwei Migration-Version-Erwartungen (3→5 in TestOpenAndMigrate, 3→4 in TestIdempotentMigrations COUNT) — minimaler Test-Update, keine Logik-Änderung — siehe Cross-Check-Scope-Audit-Sektion im Bericht. **Tests:** 14 neue über 3 Pakete grün mit `-race` — 6 store/agents_event_refs_test (Insert/Read/Idempotenz/FK/CHECK-IGNORE/MissingFields), 6 agents/read_event_refs_test (Empty/MultiRef/MissingParam/InvalidFormat/BearerAuth/AllRefTypes), 1 agents/three_source_event_refs_test (UNIQUE-Tupel collapse 3 Sources), 1 dashboard/agents_test (Event-Refs-Sub-Liste rendert mit Pill-Klassen + #event-id + Header). **DoD-Verifikation:** `go vet ./...` clean, `go test -race -count=1 ./...` 13 Pakete grün (0 Regressionen), `make hub mcp mcp-windows hub-windows` 4 saubere Binaries (ELF + PE32+, CGO-frei). **Cross-Check-Scope 21. erfolgreiche Anwendung:** `git diff 066f805 -- cmd/cli/ internal/{adb,crash,ingest,ws,config,cliconfig,http}/` = 0 Bytes verifiziert. Touch-Set: 12 modifiziert + 5 neu, alle in erlaubt-Scope (Ausnahme `sqlite_test.go` als Migration-Folge-Edit dokumentiert). Branch `feat/phase-2d-agents` bereit für Commit-Split + Sammel-Gate-Tuvok über Phase 2d gesamt.
  - 2026-05-18T (Sammel-Gate-QS) — tuvok, `qs-20260518-002`: **Sammel-Gate über Phase 2d gesamt** (S0+S1+S2+S3+S4+S5+S5-Tail = #031–#037). Status `freigabe` / Schweregrad `minor` / **0 Blocker / 0 Major / 1 Minor**: **VC-001-KON** (`docs/ARCH.md` Z.2072 sagt noch „ADR-014 (Proposed, status awaits Admin/Chakotay decision)" — Doku-Drift, ADR-014 ist seit Z.2080 Accepted). 7 Prüfgegenstände geprüft: (1) Cross-Sub-Sprint-Konsistenz inkl. ADR-013+014 beide Accepted, Migration-Chain 0001→0002→0003→0005 mit Gap-0004 dank numerischem Sort harmlos, FK-Konsistenz aller 4 Domain-Tabellen + agent_event_refs verifiziert; (2) Wire-Stability aller Phase-1/2a/2b/2c-Endpoints unverändert + 6 Read-Endpoints unter `AgentsReadMux` konsistent; (3) Cross-Compile 4 Binaries (ELF+PE32+, CGO-frei), `go vet` clean, `go test -race -count=1 ./...` 13 Pakete grün, `go mod tidy` Diff=0; (4) Push-Ready: clean working tree, branch sync mit origin, `git merge-base --is-ancestor main feat/phase-2d-agents` = true, keine Force-Pushes; (5) Domain-Komplettheit alle 4 Punkte (Spawn-Tree/Token-Usage/Verdict+Lerneffekt/Mailbox-Edges+Cross-Refs) + Phase-3-Bookmark plan-konform vermerkt; (6) Cross-Check-Scope-Audits 15-21 alle empirisch sauber (Stichprobe S5+S5-Tail verbotene Pfade = 0 Bytes verifiziert); (7) Methodik-Highlights — 3-Klauseln-Promotion verankert + angewendet, Pre-Hardcoding-Verifikation 6× erfolgreich (größter Win #036 Migration-0004-Annahme widerlegt), L26-Stub-First in dieser QS re-validiert. **Empfehlung: FF-Merge-Ready: ja.** Minor-Finding VC-001-KON nicht blockierend — empfohlen vor FF-Push als 1-Edit-Fix für ARCH-Kohärenz, kann aber auch als Tail-Cleanup nach Merge. Bericht: `XBrain/Tools/Agents/Tuvok/Reports/qs-20260518-S5-Sammel-Gate.md`.
  - 2026-05-18T (Findings-Gate) — chakotay: Sammel-Gate freigabe/minor → **Freigabe durchgewinkt**. Strategie/Proportion-Check: 1 Minor (VC-001-KON ARCH-Doku-Drift Z.2072 Inline-Referenz „Proposed" obwohl ADR-014 längst Accepted) ist trivial und legitim — fixe ich als **Lead-Direktarbeit vor FF-Push** (1-Zeilen-Edit für ARCH-Kohärenz, kein Worker-Loop nötig). Phase 2d komplett: alle 6 Sub-Sprints + Methodik-Highlights (3-Klauseln-Promotion + Pre-Hardcoding 6×) + 1 Phase-3-Bookmark dokumentiert. **FF-Merge-Approval-Frage geht an Admin** — bei „y" FF-Merge `feat/phase-2d-agents` → `main` (Force-Operation-Confirm-Pflichtig laut 5a-Default). WORKLOG-Sync `<dieser-commit>` enthält ARCH-Z.2072-Fix + WORKLOG-Stati-Update.
...
- **Status:** ✅ Sammel-Gate-grün (via #037) — D1+D3 unter Sammel-Gate qs-20260518-002 freigabe/minor freigegeben. ADR-014-Decision Option B per Admin-Confirm 2026-05-18 in #037 implementiert.
- **Verlauf:**
  ...
  - 2026-05-18T (Annahme + Worker-Spawn) — belanna: Auftrag angenommen, Klasse sprint mit 3 Deliverables (Mailbox-Edges-Persistenz + Cross-Refs Live-Tail + UI-Erweiterung) + 1 Sammel-Gate-Komponente. Worker-Spawn ballard mit Pre-Hardcoding-Verifikation 5. Anwendung als Pflicht-Erststep + 3-Klauseln-Methodik (Backlog-Hygiene + Pre-Hardcoding + Stub-First) + **Architektur-Auto-Stop bei Cross-Ref-Option-Wahl** (Option A polymorph vs Option B separate Tabelle) — ballard dokumentiert beide Optionen in ARCH.md mit ADR-014-Vorschlag und kommt sofort mit Empfehlung zurück, NICHT durchziehen. Nach Worker-Done (oder Architektur-Stop) → Tuvok release-qs als **Sammel-Gate über Phase 2d gesamt** (S0+S1+S2+S3+S4+S5) — Cross-Sub-Sprint-Konsistenz + Wire-Stability + Cross-Compile-Final + Push-Ready vor FF-Merge.
  - 2026-05-18T (Worker-Done + Architektur-Auto-Stop) — ballard: D1 (Mailbox-Edges-Persistenz) + D3 (Agents-Tab UI-Erweiterung) **erledigt**, D2 (Cross-Refs Live-Tail) per **plan-konformem Auto-Stop** vor Code-Implementierung — ADR-014 (Proposed) mit beiden Optionen + Empfehlung Option B in ARCH.md dokumentiert. **Pre-Hardcoding-Verifikation 5. erfolgreiche Anwendung** widerlegte Briefing-Annahmen massiv: (a) Tabelle `agent_mailbox_edges` ist seit S1-Migration 0003 **komplett implementiert** (Schema + InsertAgentMailboxEdge + payload-validation + MCP-Tool-Schema + client-types) — Migration 0004 NICHT erstellt, wäre Konflikt mit bestehender chain; (b) Schema-Felder sind `(id, from_spawn_id, to_spawn_id, edge_type, ts)` — NICHT `(source, payload_json, occurred_at)` wie im Brief vermutet; (c) `events.id` ist `INTEGER PRIMARY KEY AUTOINCREMENT` → stabil referenzierbar (ADR-014-Voraussetzung erfüllt). Tatsächliche Lücken: Transcript-Edge-Extraktion fehlte (jetzt da: parent→child `spawn`-Edge bei Agent/Task/Skill-tool_use + child→parent `return`-Edge bei toolUseResult), Store-Read fehlte (jetzt da: `AgentEdgesForSpawn` + batched `AgentEdgesForSpawnIDs`), Read-Endpoint fehlte (jetzt da: `GET /agents/edges?spawn_id=…` als 5. AgentsReadMux-Route), Client-Methode fehlte (jetzt da: `AgentsEdges`), UI-Sub-Liste fehlte (jetzt da: `tl-agent-edge-list` rechtsbündige Spalte mit ←/→-Marker). **Tests:** 17 neue über 4 Pakete grün mit `-race` — 6 store/agents_edges_test.go, 6 agents/read_edges_test.go, 1 agents/three_source_edges_test.go (UNIQUE-Tupel collapse über alle 3 Sources), 3 agents/transcript_edges_test.go (spawn-edge + return-edge + persistEvent-dispatch), 1 dashboard/agents_test.go (Edges-Sub-Liste rendert beide Richtungen). **Cross-Check-Scope 20. erfolgreiche Anwendung:** 0 Bytes Diff in `cmd/cli`, `internal/{adb,crash,ingest,ws,config,cliconfig,http}` empirisch verifiziert via `git diff b5478bf -- <verbotene>`. **DoD-Verifikation:** `go vet ./...` clean, `go test -race ./...` 13 Pakete grün, `make hub mcp mcp-windows hub-windows` 4 saubere Binaries (ELF + PE32+, CGO-frei). **ADR-014-Empfehlung Option B (separate `agent_event_refs`-Tabelle)** mit 3 Begründungen (additive vs. destruktive Migration / Domain-Klarheit spawn↔spawn vs spawn↔event / `/agents/edges`-API-Stabilität bleibt). Auto-Stop ist plan-konform — nicht Fehler-Status, sondern Architektur-Entscheidung wartet auf Admin/Chakotay.
  - 2026-05-18T (Eskalations-Bericht-Annahme + Routing an Admin) — chakotay: belanna task-20260518-002 empfangen, Status `eskalation` (plan-konform, kein Fehler). D1+D3 ✅ vollständig (push synchron `3d691d0`), D2 → Architektur-Auto-Stop für ADR-014 wie eingeplant. Pre-Hardcoding-Verifikation 5. Anwendung = größter Win bisher — destruktive Falsch-Migration 0004 verhindert (Schema war seit S1/Migration 0003 da). ADR-014-Frage (Option A polymorph vs Option B separate `agent_event_refs`-Tabelle, Worker-Empfehlung Option B mit 3 Begründungen) + Sammel-Gate-Strategie-Frage (jetzt-für-D1+D3 vs erst-nach-D2-komplett) werden an Admin eskaliert — Architektur-Tragweite > Lead-Autonomie. WORKLOG-Sync `<dieser-commit>`.
...
- **Status:** ✅ QS-grün — bereit für Chakotay-Findings-Gate
- **Verlauf:**
  ...
  - 2026-05-17T (Annahme + Worker-Spawn) — belanna: Auftrag angenommen, Klasse sprint mit 4 Deliverables (Read-API + Tab-Funktionalisierung + Tests + ARCH-Erweiterung). Worker-Spawn ballard mit Pre-Hardcoding-Verifikation 4. Anwendung als Pflicht-Erststep (Read-Surface-Existenz + Store-Read-Queries + Tree-Aufbau-Strategie + Phase-2c-UI-Konventionen + Test-Fixtures-Pattern). 3-Klauseln-Methodik (Backlog-Hygiene + Pre-Hardcoding + Stub-First) referenziert. Auto-Stop-Trigger an ballard durchgereicht. Nach Code-Done → Tuvok release-qs für Read-API-Konsistenz + UI-Konvention + Cross-Source-Aggregation + Wire-Stability.
  - 2026-05-18T (Worker-Done) — ballard: alle 4 DoD-Items erledigt. (1) Read-API-Endpoints **neu** (Pre-Hardcoding bestätigte: 0 Read-Surface aus S1-S3 — Handler/Store comments dokumentierten „reserved for S3+") via Overlay-Mux in `cmd/hub/main.go` vor chi (internal/http strikt 0-Bytes). (2) `tab_agents.gohtml` voll funktionalisiert mit Spawn-Tree (BFS + tree-glyph indent), per-Source-Token-Breakdown, Verdict-Pill, Lerneffekt-Disclosure. (3) Tests: 19 neue (5 store + 9 agents/read + 5 dashboard/agents + 5 client/agents-Read), gesamt 13 Pakete grün mit `-race`. (4) ARCH.md §Read-Surface S4 mit 4 Endpoint-Tabelle + Response-Shapes + Wireup-Posture-Erklärung + Tree-Strategie-Begründung. **E2E gegen Live-Daemon `127.0.0.1:28770`**: 3-Source-Fixture (sdk-hook 1000/500 + transcript 1100/550 + mcp-push 1050/525) → totals.input_tokens=3150, by_source mit 3 Keys, BFS-Tree depth=0/1, freigabe-verdict, Dashboard rendert alles korrekt. Cross-Check-Scope 19. Anwendung: 0 Bytes Diff in `cmd/{cli,mcp}` und `internal/{adb,crash,ingest,ws,config,cliconfig,http}` empirisch verifiziert; S1-S3-Owner-Files `handler.go`/`payload.go`/`transcript.go`/`transcript_test.go`/`handler_test.go`/`three_source_test.go` ebenfalls 0 Bytes. Pre-Hardcoding-Verifikation 4. erfolgreiche Anwendung — verhinderte: (a) Annahme „Read-Surface könnte schon da sein" → strikt empirisch widerlegt (komplett neu), (b) Annahme „Route-Wireup in cmd/hub/main.go = einfaches Append" → Auflösung: Wrapper-Mux vor chi statt internal/http-Edit, weil Cross-Check-Scope-Brief das verbietet. Branch lokal — push wartet auf Belanna-Annahme.
  - 2026-05-18T (Commit-Split + Push) — belanna: ballards 15 Files in 6 atomare Commits zerlegt + nach origin gepusht (`0b9457f` Worklog-Worker-Done · `bb2ec87` feat(store)/Read-Queries · `a6fa7da` feat(agents)/Read-API+Overlay · `862e792` feat(dashboard)/Agents-Tab funktional · `88ab386` feat(client)/4 Read-Methods · `b6bddfe` docs(arch)/§Read-Surface). Branch `feat/phase-2d-agents` HEAD `b6bddfe`, push synchron mit origin. Tuvok release-qs-Spawn (Klasse release-qs, Stub-File-First per L26 — Bericht-File `XBrain/Tools/Agents/Tuvok/Reports/qs-20260518-S4.md`).
  - 2026-05-18T (QS-grün) — tuvok, qs-20260518-001: Status `freigabe` / Schweregrad `none` / 0/0/0 Findings über alle 4 DoD-Prüfgegenstände + **7 Plus-Befunde**: (1) `errorBody`-Helper DRY-Mini, (2) `panic`-Fail-Fast bei leerem Token, (3) `isValidSpawnIDFormat`-Defence-in-Depth, (4) `parseLimitParam` 400-frei (Browser-Render-Schutz), (5) `computeVisiblePageDepth` Pass-Iteration mit Early-Break, (6) „Coming Soon"-Zombie-Test entsorgt, (7) canonical source-order im Breakdown. Bearer-Auth-Algorithmus in `AgentsReadMux` **algorithmisch identisch** zu `internal/http/bearerAuth` (constant-time-compare, identischer 401-Body) — Code-Duplikation als kalkulierte Cross-Check-Scope-Schuld im ARCH dokumentiert + Future-Merge-Bookmark gesetzt. `tl-empty-state`/`tl-error-card` S4-Erstanwendung als bewusste richere-Block-Variante neben legacy `tl-empty` inline-marker, kein Drift. Tree-Strategie App-Level-BFS mit projekt-skopiertem Working-Set + Visited-Set + ADR-Revisit-Marker bei >10k Spawns. Cross-Check-Scope 19. Anwendung empirisch sauber via `git diff 1f03d00..b6bddfe --stat`. 13 Pakete grün mit `-race`, 4 CGO-freie Binaries (ELF+PE32+). Standard-Durchwink-Empfehlung. **S5-Strategie-Frage gehört zum Findings-Gate** (Plan-explizit kein Auto-Chain — Sammel-Gate-Charakter).
...
- **Status:** ✅ QS-grün — wartet auf Admin-S3-Strategie-Entscheidung
- **Verlauf:**
  ...
  - 2026-05-17T (Worker-Done) — ballard: 5 Commits `9d6727b..017b8e8` auf `feat/phase-2d-agents` gepusht. **Highlight (2. Pre-Hardcoding-Verifikation-Anwendung):** Read auf reale Session-JSONLs widerlegte 3 Worker-Brief-Annahmen — (a) `sessionId` ist Claude-Code-Conversation-UUID NICHT tracelab-session-id (FK-Violations vermieden, SessionRef leer), (b) Subagent-Streams unter `<session>/subagents/agent-<hex>.jsonl` mit `isSidechain:true` (Subagent-Tokens wären sonst verfehlt), (c) Verdict via `toolUseResult.{status,agentId,totalTokens,usage}` im PARENT-Stream (nicht im Subagent-Stream selbst). 13 Pakete grün, 4 CGO-freie Binaries, E2E-Live-Smoke `127.0.0.1:28767` mit Multi-Source-Forensik (sdk-hook 999/1 + transcript 1/500 nebeneinander).
  - 2026-05-17T (QS-Lauf) — tuvok: release-qs-Spawn für 10 Prüfgegenstände (Pre-Hardcoding-Re-Verifikation + Tail-Bridge-Code-Review + Test-Coverage + Config + Hub-Wireup + ARCH-Erweiterung + Multi-Source-Idempotenz + Cross-Check-Scope + Build/Test/Cross-Compile + Wire-Stability).
  - 2026-05-17T (QS-grün) — tuvok, qs-20260517-004: Status `freigabe` / Schweregrad `none` / 0/0/0 Findings über alle 10 Prüfgegenstände + **7 Plus-Befunde**: (1) Pre-Hardcoding-Verifikation 2. Major-Bug-Verhinderer-Anwendung, (2) Direct-Store-Call-Architektur mit Atomicity-Argument, (3) JSONL-Mapping-Tabelle 16 Felder mit Notes-Spalte, (4) 5-Limitations-Sektion explizit nummeriert, (5) `mapToolResultStatusToVerdict` Forward-Compat, (6) `TestTranscriptMultiSourceCoexistence` pinnt ADR-013-Forensik-Contract, (7) S1-Wire-Stability empirisch verifiziert. S3-Auto-Chain-Empfehlung + Methodik-Promotion-Empfehlung (Pre-Hardcoding-Verifikation 2× empirisch belegt → Worker-Brief-Konventionen.md fällig).
  - 2026-05-17T (Findings-Gate) — chakotay: freigabe/none → Standard-Durchwink. **Auto-Chain-Check:** Plan-File `~/.claude/plans/tracelab-phase-2d-agents.md` hat kein `auto-chain: true`-Frontmatter → Auto-Chain unterdrückt, klassischer routing-abschluss-Bericht an Admin mit S3-Strategie-Frage (S3 sofort vs Methodik-Sweep Worker-Brief-Konventionen.md zuerst). WORKLOG-Sync `<dieser-commit>`.
  - 2026-05-17T (Annahme + Worker-Spawn) — belanna: Auftrag angenommen, Klasse sprint mit 8 Deliverables (Pre-Hardcoding-Verifikation + Tail-Bridge + Config + Hub-Wireup + Multi-Source-Verifikation + Tests + ARCH-Doku + Fixtures). Worker-Spawn ballard mit Pre-Hardcoding-Verifikation als Pflicht-Erststep (gleiches Pattern wie S1, das einen Major-Bug verhindert hat).
  - 2026-05-17T (Worker-Done) — ballard: 4 Commits `9d6727b..0345a72` auf `feat/phase-2d-agents`. **Highlight:** Pre-Hardcoding-Verifikation des Transcript-JSONL-Formats via Read auf real-Transcripts unter `~/.claude/projects/-home-kaik-Projekte-tracelab/*.jsonl` — Annahmen aus Worker-Brief widerlegt (a) `sessionId` ist Claude-Code-Conversation-ID, NICHT `tracelab sessions.id` → SessionRef bewusst leer (b) Subagent-JSONL liegt unter `<session>/subagents/agent-<id>.jsonl` mit eigenem `agentId`-Feld (c) Spawn-End + Verdict + Summary-Usage kommen via `toolUseResult.{status,agentId,totalTokens}` im PARENT-Stream, nicht im Sub-Stream. Hätte ich blind hardcoded, wären (a) → FK-violations bei jedem Insert (b) → subagent-Stream nie entdeckt (c) → verdict-rows fehlend. Pattern aus S1 wieder bestätigt: bei externen JSONL-Formaten Sample-Inspection vor Mapping. Architektur-Entscheidung Option A (Erweiterung `internal/agents/`) + Polling-Tail (statt fsnotify) + direkter Store-Call (statt HTTP-Self-Post) dokumentiert im File-Header von `transcript.go` und in ARCH.md. **E2E-Live-Smoke gegen tmp-Daemon mit `cfg.Agents.Transcript.{Enabled=true,ProjectsRoot=/tmp/...,PollIntervalMs=200}`:** sample-jsonl copy → 3 spawns + 3 tokens(source=transcript) + 1 verdict(freigabe) sichtbar in DB; idempotenter Re-Tail nach 1 s → counts unverändert; POST /agents/ingest mit sdk-hook-source gegen denselben spawn-id → `spawns=0,tokens=1` (PK collapse + UNIQUE-tuple keeps beide Sources) → DB zeigt sdk-hook 999/1 nebeneinander mit transcript 1/500 (forensisches Per-Source-Breakdown live verifiziert); corrupt-line zwischen 2 good-lines → late-good-record persistiert (corrupted-tolerance); SIGTERM-Shutdown sauber in Reihenfolge `adb→transcript→ws→http`. **DoD:** `go vet ./...` clean, `go test -race -count=1 ./...` über 14 Pakete grün (12 alte + agents nun mit +10 transcript-tests + config +3), `go mod tidy` Diff=0 (stdlib only — bufio/encoding/json/os/path/filepath/sync/time, keine neuen Deps), `make hub mcp mcp-windows hub-windows` 4 saubere Binaries. Cross-Check-Scope 17. erfolgreiche Anwendung — S1-Pakete (`internal/agents/{handler,payload}.go`, `internal/store/agents.go`) 0 Bytes Diff, ebenso alle Phase-1/2a/2b/2c-Pakete.
...
- **Verlauf:**
  ...
  - 2026-05-16T (Worker-Done) — ballard: alle 3 Bausteine implementiert. **B1 Polish:** `dashboard.css` von 306 auf 405 Zeilen konsolidiert (CSS-Variablen, `tl-tab-panel`-shared rules für Forms/Tables, `tl-table-wrap`-Wrapper, Tap-Target-Floor 44px, Schrift-Floor 14px, `tl-empty-state`/`tl-error-card`-Patterns), Tabellen in allen 4 Templates mit `tl-table-wrap` umschlossen. Mobile-Entscheidung: horizontal-scrollbar Wrapper statt Card-Collapse (begründet im Bericht: dichte tabulare Daten, Wrapper-Pattern minimal-invasiv). **B2 Agents-Stub:** `tab_agents.gohtml` neu — Phase-2d-Bookmark mit 4-Punkt-Roadmap (Skill-Spawn-Tree/Token-Usage/Verdict/Mailbox-Edges), Tab-Nav unverändert (slug bereits seit S1 in `tabs`-slice), `TestAgentsTabRendersComingSoon` ergänzt. **B3 Defense-in-Depth-Rückport:** `SessionDetailHandler` bekommt `isValidSessionIDFormat`-Layer-3 (charset `[A-Za-z0-9_-]`, Length-Cap 128, tolerant für ULID/KSUID/NanoID-Migrationen — Pragmatik > Strenge), `TestSessionDetailDefenseInDepth` mit 5 Sub-Tests (empty/embedded-slash/dotdot/space/pipe). Cross-Check-Scope (13. Anwendung): 0 Bytes Diff in `cmd/{cli,mcp,hub}` + `internal/{adb,client,config,cliconfig,crash,ingest,ws}`. `go vet`/`go test -race ./...` repo-weit grün (12 Pakete), `go mod tidy` Diff=0, `make hub mcp mcp-windows hub-windows` 4 saubere Binaries CGO-frei. E2E-Smoke auf `127.0.0.1:28767` mit seed-session + Java-NPE-Crash: alle 4 Tabs HTTP 200, Agents zeigt „Phase 2d/coming soon/Skill-Spawn-Tree", Defense-in-Depth: empty/embedded-slash/`..`/`%20`/unknown alle 404, valid-existing 200. Commits: `a350ace`+`578b332`+`b93b909` 3-atomic-Split, gepusht. **Bookmark für Tuvok-Sammel-Gate:** Mobile-DevTools-Smoke 375px auf laufendem Daemon visuell verifizieren (Code-Audit hat alle Mobile-Konventionen erfüllt, aber visueller Pass steht aus). Status: erledigt — wartet auf Sammel-Gate-QS via belanna→tuvok.
  - 2026-05-16T (Sammel-Gate-Spawn) — belanna: Tuvok release-qs für **Cross-Phase-QS über S1+S2+S3+S4+S5 gesamt** gestartet (nicht nur S5-Sub-Sprint-QS). Pflicht-Scope: alle 6 DoD-Punkte (a-f) aus Plan-Briefing 2c Block 1, Cross-Sub-Sprint-Konsistenz, Wire-Stability, Cross-Compile-Final, Branch-Push-Ready für FF-Merge, plus Mobile-DevTools-Smoke 375px aus Worker-Bookmark.
  - 2026-05-16T (Sammel-Gate abgeschlossen) — tuvok, qs-20260516-005: Status `freigabe` / Schweregrad `minor` / **1 Minor-Finding VC-029-WAR**. Cross-Phase-QS über alle 5 Sub-Sprints live verifiziert gegen seeded Daemon `127.0.0.1:28768`. **DoD (a-f) alle ✅:** /dashboard rendert (Layout-Klassen + htmx/SSE Scripts), SSE-Live-Tail E2E mit parallel-Ingest sichtbar, Session-Browser+Detail funktional, Crash-Inspector mit Fingerprint+Top-Frames+Dedup live, Agents-Tab zeigt „Phase 2d — coming soon"-Stub, `go test -race ./...` 12 Pakete grün + `go vet` + `go mod tidy` Diff=0. **Cross-Sub-Sprint-Konsistenz:** Tab-Templates konsistent `tl-tab-panel`/`data-tab`/`role="tabpanel"`, Pagination identisch in sessions+crashes, `tl-table-wrap` flächendeckend retrofitted (S3+S4) + neu (S5), htmx-Trigger-Konvention konsistent (sort/filter/session-dropdown), Defense-in-Depth-Layering konsistent zwischen `SessionDetailHandler` (S5-Rückport, 4 Layer) und `CrashDetailHandler` (S4-original, 4 Layer) — Live-Smoke beider bestätigt empty/embedded-slash/dotdot/whitespace/unknown → 404. **Wire-Stability:** 9 Endpoints (`/healthz`, `/sessions`, `/events`, `/crashes`, `/session/{start,end}`, `/ingest`, `/tail`) Spot-Check gegen Phase-2b-Stand Drift=0. **Cross-Compile-Final:** `make clean && make hub mcp mcp-windows hub-windows` 4 Binaries CGO-frei (ELF + PE32+ per `file` bestätigt). **Branch-Push-Ready:** `git merge-base --is-ancestor main HEAD` = true, kein origin-Drift, 21 Commits `main..HEAD` plan-konform. **ADR-Closure:** ADR-011 + ADR-012 beide Accepted mit befülltem Decision/Consequences-Block — ADR-Methodik aus qs-002 hier 3.→4. Anwendung. **Cross-Check-Scope-Pattern 13× ungebrochen** — promotion-reif zu eigener `30_Wissen`-Note (Tuvok-Empfehlung).
