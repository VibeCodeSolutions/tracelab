...
- **Verlauf:**
  ...
  - 2026-05-16T (Worker-Done) — ballard: alle 3 Bausteine implementiert. **B1 Polish:** `dashboard.css` von 306 auf 405 Zeilen konsolidiert (CSS-Variablen, `tl-tab-panel`-shared rules für Forms/Tables, `tl-table-wrap`-Wrapper, Tap-Target-Floor 44px, Schrift-Floor 14px, `tl-empty-state`/`tl-error-card`-Patterns), Tabellen in allen 4 Templates mit `tl-table-wrap` umschlossen. **B2 Agents-Stub:** `tab_agents.gohtml` neu — Phase-2d-Bookmark mit 4-Punkt-Roadmap (Skill-Spawn-Tree/Token-Usage/Verdict/Mailbox-Edges), Tab-Nav unverändert (slug bereits seit S1 in `tabs`-slice), `TestAgentsTabRendersComingSoon` ergänzt. **B3 Defense-in-Depth-Rückport:** `SessionDetailHandler` bekommt `isValidSessionIDFormat`-Layer-3 (charset `[A-Za-z0-9_-]`, tolerant für ULID/KSUID/NanoID-Migrationen), `TestSessionDetailDefenseInDepth` mit 5 Sub-Tests ergänzt. Cross-Check-Scope (13. Anwendung): 0 Bytes Diff in `cmd/{cli,mcp,hub}` + `internal/{adb,client,config,cliconfig,crash,ingest,ws}`. `go vet`/`go test -race ./...` repo-weit grün (12 Pakete), `go mod tidy` Diff=0, `make hub mcp mcp-windows hub-windows` 4 saubere Binaries CGO-frei. E2E-Smoke auf `127.0.0.1:28767` mit seed-session + crash: alle 4 Tabs HTTP 200, Agents zeigt „Phase 2d/coming soon/Skill-Spawn-Tree", Defense-in-Depth: empty/embedded-slash/`..`/`%20`/unknown alle 404, valid-existing 200. Status: erledigt — wartet auf Sammel-Gate-QS via belanna→tuvok.
